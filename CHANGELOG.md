...
- `GET /api/v1/artifacts/{uri}` redirects to a presigned URL of five minutes, or serves the bytes where the artifact has a fetch budget, counting a fetch once the whole of them has gone; 410 once expired or spent. `api.OnArtifact` authorises it with `run:read_data` on the run the URI names.
