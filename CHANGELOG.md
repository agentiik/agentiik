...
- `Bus.Stops` hands a runner every stop on `agentiik.stops` over the connection `bus.OpenRunner` opened, answering once the server holds the subscription and refusing a credential that may not hear stops. A stop it cannot read is reported and passed over.
