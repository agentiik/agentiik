...
- `agk-runner join --api <url> --token <token>` generates the host's Ed25519 key, measures its vCPU, memory (`/proc/meminfo`) and the disk available under `AGK_RUNNER_WORKDIR`, reads the daemon's runtime and remapping, and joins with the labels of `--labels` or `AGK_RUNNER_LABELS` and the narrowing of `AGK_RUNNER_NAMESPACES`. It writes the key to `/var/lib/agentiik/runner.key` and `AGK_API`, `AGK_RUNNER_ID`, `AGK_RUNNER_POOL`, `AGK_RUNNER_LABELS`, `AGK_RUNNER_NAMESPACES` and `AGK_RUNNER_CREDENTIAL` to `/etc/agentiik/runner.env`, each mode 0600, given to `--user` (`agentiik`) when run as root, and moved into place whole once the API has answered. A refused token writes nothing, a flag its environment disagrees with is refused before the token is spent, an identity already there is replaced only with `--replace` (the old `runner.env` removed first), and an answer `serve` could not read is never written.
