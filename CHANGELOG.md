...
- A CI job of its own sets the daemon to `userns-remap: default`, mounts a secrets tmpfs and runs the driver's remapped-daemon tests under `setpriv` with the three capabilities, with `AGENTIIK_TEST_REQUIRE_USERNS=1` so they cannot skip. Elsewhere they skip and say why.
