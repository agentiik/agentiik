...
- A redemption binds its task only once it has an answer to give. A secret the store cannot give, or an input envelope it cannot read, is refused and leaves the task for the next runner, as a missing tree already did. The values are read last, once nothing else can refuse, and in no transaction.
...
- A test holds that every redemption reads each secret its grant names from the store again, in the grant's namespace and nothing else, so a value rotated in between arrives rotated, and that no value it answers is written anywhere in the database.
