...
- A revoked runner is heard until the revocation plus `api.RunnerOptions.RevocationGrace` (`AGK_REVOCATION_GRACE`, an hour where unset): its heartbeat answers `drain: true` and `results_accepted_until`, no later than its `rotate_by`, and keeps its tasks from being lost, and its bus credential publishes its results and hears stops, and pulls nothing. It rotates nothing (403). From the end of the grace every call is 401, and the sweep declares its tasks lost. A draining runner keeps its bus credential and rotates.
- A redemption by a draining or revoked runner is 403 and binds nothing, checked again at the binding, so the runner puts the message back for another runner of the pool.
