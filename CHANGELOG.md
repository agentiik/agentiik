...
- Only a loss counts against `max_requeues`. A message the bus hands on because its runner died before redeeming it is the same dispatch delivered again, and a task waiting on a full pool's queue is never lost, so neither spends a requeue.
