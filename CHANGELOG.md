...
- A runner's secrets directory must be a tmpfs mounted `noexec,nosuid,nodev`, read with `statfs` when the daemon is opened and again before a value is written; otherwise it is `driver.ErrSecretsTmpfsRequired`. A secret is a bind from there and keeps those flags. `Policy.RequireSecretsTmpfs` is the floor; no line of `runner.toml` lifts it, and `agk run --local`, `agk brick test` and `agk validate` do.
