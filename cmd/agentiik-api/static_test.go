...
func TestTheBinaryIsAStaticLinuxELFForEveryArchitectureTheReleaseBuilds(t *testing.T) {
	for _, c := range architectures {
		t.Run(c.arch, func(t *testing.T) {
			f, err := elf.Open(buildAPI(t, c.arch))
			if err != nil {
...
func TestTheImageRunsTheBinaryAsAUserThatIsNotRoot(t *testing.T) {
	...
	t.Cleanup(func() { exec.Command("docker", "image", "rm", "-f", image).Run() })
	...
	// The object store's directory and the bus identity's are the image user's, so that a named
	// volume mounted at either takes that owner, and the bus identity's is that user's alone, since
	// bus-init refuses a directory anybody else may write to.
	for path, mode := range map[string]os.FileMode{"/var/lib/agentiik/objects": 0, "/var/lib/agentiik/bus": 0o700} {
		archive, err := exec.Command("docker", "cp", id+":"+path, "-").Output()
		if err != nil {
			t.Fatalf("the image holds no %s: %v", path, err)
		}
		...
		if owner := fmt.Sprintf("%d:%d", header.Uid, header.Gid); owner != cfg.User || !header.FileInfo().IsDir() {
			t.Errorf("%s is owned by %s, and the image runs as %s", path, owner, cfg.User)
		}
		if got := header.FileInfo().Mode().Perm(); mode != 0 && got != mode {
			t.Errorf("%s is mode %v, want %v", path, got, mode)
		}
	}
}
...
// build/api.Dockerfile gives for the binaries the image is built from, and the two are kept the
// same by hand until a release workflow builds from one definition of them. A machine with no Go
// toolchain in reach skips.
