...
func (l *Loop) carry(ctx context.Context, t bus.Taken) {
	...
	case errors.Is(err, driver.ErrTaskInFlight):
		...
		return
	case err != nil:
		l.say(fmt.Sprintf("task %s (%s) is put back, since its key could not be written down: %s", m.TaskID, m.IdempotencyKey, err))
	...
	case next == RedeemPutBack:
		l.Holder.Release(id)
		l.say(fmt.Sprintf("task %s (%s) is put back for another runner of the pool: %s", m.TaskID, m.IdempotencyKey, err))
		if err := t.Again(); err != nil {
			l.say(err.Error())
		}
