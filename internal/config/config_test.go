...
func (i *installation) onlyWhatIsRequired() {
	for _, name := range []string{
		...
		config.RevocationGrace, config.OTLPEndpoint,
	} {
		delete(i.env, name)
	}
}
...
func TestAWholeInstallationIsRead(t *testing.T) {
	...
	want := config.Controller{Database: wantDatabase, Bus: wantBus, Objects: i.env[config.ObjectsDir], MaxRequeues: 1, TaskCeiling: 2 * time.Hour, OTLPEndpoint: "http://127.0.0.1:4318"}
	if controller != want {
...
func TestEachProgramReadsOnlyWhatItNeeds(t *testing.T) {
	neverAsked := map[string][]string{
		theAPI.name: {config.MaxRequeues, config.MigrateDatabaseURL, config.MigrateDatabasePasswordFile, config.OTLPEndpoint},
		theController.name: {
		...
		migrating.name: {
			...
			config.JoinRotation, config.RevocationGrace, config.OTLPEndpoint,
		},
	}
	i := anInstallation(t)
