...
require (
	...
	// the API" that a runner reaches the bus with, and to sign the operator and the accounts
	// an installation's bus trusts. This is the library the server verifies with, so the
	// claims this module writes and the claims the far end reads are the same structure
	// rather than two readings of a specification. Hand-rolling a JWT for somebody else's
	// verifier is the kind of clever that is wrong once and wrong for ever.
	...
	// pair and a JWT naming its public half, and this is what mints one, what makes the
	// operator and account keys an installation is created with, and what signs with them.
